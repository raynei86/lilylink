    ...
    (labels ((peekc ()
             ...
             (skip-comment ()
               ...
               (if (and (peekc) (char= (peekc) #\{))
                   ...
                   (iter (until (or (eofp) (char= (peekc) #\Newline)))
                         (consume))))
             (scan-digits-and-dots ()
               ...
               (when (and (peekc) (digit-char-p (peekc)))
                 (let ((digits nil)
                       (dots 0))
                   (iter (while (and (peekc) (digit-char-p (peekc))))
                         (push (consume) digits))
                   (iter (while (and (peekc) (char= (peekc) #\.)))
                         (consume)
                         (incf dots))
               ...
               (let ((c (char run 0)))
                  (cond
                    ...
                    (multiple-value-bind (step alter fullp) (note-parts run)
                      (if (and step fullp)
                          (let ((mark 0))
                            (iter (while (and (peekc) (member (peekc) '(#\' #\,))))
                                  (if (char= (consume) #\')
                                      (incf mark)
                                      (decf mark)))
             ...
             (scan-string ()
               ...
               (let ((str (with-output-to-string (s)
                            (iter (while (and (peekc) (not (char= (peekc) #\"))))
                                  (let ((c (consume)))
                                    (if (char= c #\\)
                                        (write-char (consume) s)
                                        (write-char c s)))))))
