...
(deftest tokenize-notes
  ...
  (testing "octave marks and duration are captured"
    (let* ((tokens (lilylink:tokenize "c'4 c, c''8."))
      ...
      (ok (= (lilylink::pitch-token-octave-mark (first vals)) 1))
      (ok (= (lilylink::pitch-token-octave-mark (second vals)) -1))
      (ok (= (lilylink::pitch-token-octave-mark (third vals)) 2))
      (ok (= (car (lilylink::pitch-token-duration (third vals))) 3))
      (ok (= (cdr (lilylink::pitch-token-duration (third vals))) 1))))
  (testing "accidentals are captured"
    (let* ((tokens (lilylink:tokenize "cis aes ees cisis?"))
           (alters (mapcar (lambda (tok)
                             (lilylink::pitch-token-alter (lilylink::token-value tok)))
                           tokens)))
      (ok (= (length alters) 4))
